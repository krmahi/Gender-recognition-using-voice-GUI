...
location = args[1]
soundLocation = paste(location, "/sounds", sep = "") # nolint
textLocation = paste(location, "/output", sep = "") # nolint
...
suppressMessages({
  library(NatureSounds, quietly = TRUE)
...
specan3 <-
  function(X,
           ...
           threshold = 5,
           parallel = 1) {
    #To use parallel processing: library(devtools), install_github('nathanvan/parallelsugar') # nolint # nolint
    ...
    if (class(X) == "data.frame") {
                ...
                "start", "end") %in% colnames(X)))
      {
        ...
        sound.files <- as.character(unlist(X$sound.files)) # nolint
        selec <- as.character(unlist(X$selec))
      } else
        stop(paste(paste(
          c("sound.files", "selec", "start", "end")[!(c("sound.files", "selec",
                                                        "start", "end") %in% colnames(X))], collapse = # nolint
            ", "
    ...
    x <- as.data.frame(lapp(1:length(start), function(i) { # nolint
      r <-
        tuneR::readWave(
      ...
      Q25 <- analysis$Q25 / 1000 # nolint
      Q75 <- analysis$Q75 / 1000 # nolint
      IQR <- analysis$IQR / 1000 # nolint
      skew <- analysis$skewness
      kurt <- analysis$kurtosis
      sp.ent <- analysis$sh # nolint
      sfm <- analysis$sfm
